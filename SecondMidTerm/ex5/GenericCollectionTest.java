...
class GenericCollection<T extends IHasTimestamp> {
    public Map<String, List<T>> collectionOfGenerics;

    public GenericCollection() {
        this.collectionOfGenerics = new HashMap<>();
    }

    public void addGenericItem(String category, T element) {
        collectionOfGenerics.computeIfAbsent(category, x -> new ArrayList<>()).add(element);
    }

    public Collection<T> findAllBetween(LocalDateTime from, LocalDateTime to) {
        return collectionOfGenerics.values()
                .stream()
                .flatMap(List::stream)
                .filter(t -> (t.getTimestamp().isAfter(from) && t.getTimestamp().isBefore(to))).sorted().collect(Collectors.toCollection(ArrayList::new));
    }

    public Collection<T> itemsFromCategories(List<String> categories) {
        return collectionOfGenerics.entrySet()
                .stream()
                .filter(x -> categories.contains(x.getKey()))
    ...
    public Map<String, Set<T>> byMonthAndDay() {
        Map<String, Set<T>> groupedByMonthAndADay = new TreeMap<>();
        collectionOfGenerics.values()
                .stream()
                .flatMap(List::stream)
                .forEach(line -> groupedByMonthAndADay.computeIfAbsent(line.getTimestamp().getMonth().getValue() + "-" + line.getTimestamp().getDayOfMonth(), x -> new HashSet<>()).add(line));
        return groupedByMonthAndADay.entrySet()
                .stream()
    ...
    public Map<Integer, Long> countByYear() {
        Map<Integer, Long> yearCreated = new TreeMap<>();
        collectionOfGenerics.values().stream()
                .flatMap(List::stream)
                .forEach(line -> {
                    if (yearCreated.containsKey(line.getTimestamp().getYear())) {
                        long frequency = yearCreated.get(line.getTimestamp().getYear());
                        frequency++;
                        yearCreated.put(line.getTimestamp().getYear(), frequency);
                    } else {
                        yearCreated.put(line.getTimestamp().getYear(), 1L);
                    }
                });
        return yearCreated;
    }
